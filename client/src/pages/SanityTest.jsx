...
import { Database, CheckCircle, XCircle, AlertTriangle, RefreshCw, Server, Image as ImageIcon, Box } from 'lucide-react';
...
const HealthIndicator = ({ status, label, details }) => {
    ...
    return (
        <div className={`p-4 rounded-lg border ${bgColor} flex items-start gap-4 mb-4 transition-all duration-300`}>
            <div className={`mt-1 ${color}`}>{icon}</div>
            <div className="flex-1">
                <h3 className={`font-bold ${color}`}>{label}</h3>
                <p className="text-sm text-gray-300 mt-1 font-mono whitespace-pre-wrap">{details}</p>
            </div>
        </div>
    );
};
...
const SanityHealthCheck = () => {
    const [mongoStatus, setMongoStatus] = useState({ status: 'loading', message: 'Connecting...' });
    const [sanityStatus, setSanityStatus] = useState({ status: 'loading', message: 'Connecting...' });
    const [assetStatus, setAssetStatus] = useState({ status: 'loading', message: 'Checking assets...' });
    ...
    const testMongo = async () => {
        ...
        try {
            ...
            if (data.success) {
                setMongoStatus({ status: 'success', message: 'Connected to MongoDB Atlas successfully.' });
            } else {
                setMongoStatus({ status: 'error', message: data.error || 'Unknown database error' });
            }
        } catch (err) {
    ...
    const testSanity = async () => {
        setSanityStatus({ status: 'loading', message: 'Verifying Content Lake...' });
        try {
            // Fetch Site Settings as a connectivity check
            const settings = await client.fetch('*[_type == "siteSettings"][0]{title}');
    ...
    const testAssets = async () => {
        setAssetStatus({ status: 'loading', message: 'Verifying Image Pipeline...' });
        try {
            // Fetch the first product image to verify CDN
            const product = await client.fetch('*[_type == "product" && defined(mainImage)][0]{ "url": mainImage.asset->url }');
            ...
            if (product && product.url) {
                ...
                setAssetStatus({
                    status: 'warning',
                    message: 'No product images found to test. Add products with images in CMS.'
                });
            }
        } catch (err) {
            setAssetStatus({ status: 'error', message: `Asset Check Failed: ${err.message}` });
        }
    };
        ...
        <div className="min-h-screen bg-black text-white p-6 pt-32 flex flex-col items-center font-sans">
            <div className="max-w-3xl w-full">
                ...
                <div className="mb-8 text-center">
                    <h1 className="text-3xl font-serif text-royal-gold mb-2">System Diagnostic Dashboard</h1>
                    <p className="text-gray-400">Real-time health check for all application services</p>
                </div>
                ...
                <div className="bg-gray-900 border border-white/10 rounded-xl p-6 shadow-2xl">
                    ...
                    <div className="mt-8 p-4 bg-black/50 rounded border border-white/5 text-xs text-gray-500 font-mono">
                        <p className="mb-2 uppercase tracking-wider font-bold text-gray-400">Environment Details</p>
                        <div className="grid grid-cols-2 gap-2">
                            <div>API_URL: {import.meta.env.VITE_API_URL || '(default local)'}</div>
