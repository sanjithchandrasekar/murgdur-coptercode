...
const Shop = () => {
  ...
  const handleTabChange = (tab) => updateParams({ type: tab, cat: null });
      ...
      <div className="container mx-auto px-4 md:px-8 relative">
        ...
        <div className="flex flex-col items-center justify-center text-center mb-12">
          ...
          <p className="text-gray-500 max-w-lg mx-auto text-sm md:text-base italic tracking-wide mb-10">
            Explore our curated selection of {filteredProducts.length} premium
            items, designed for modern elegance.
          </p>
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {["Men", "Women", "Accessories"].map((cat) => (
                        ...
                        >
                          <div
                            className={`w-5 h-5 border flex items-center justify-center transition-all duration-300 rounded-sm ${selectedCategories.includes(cat) ? "bg-black border-black" : "border-gray-300 group-hover:border-gray-500"}`}
                          >
                          ...
                          <span
                            className={`text-sm tracking-wide transition-colors ${selectedCategories.includes(cat) ? "text-black font-semibold" : "text-gray-500 group-hover:text-black"}`}
                          >
                            {cat}
                          </span>
                          <input
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {PRICE_RANGES.map((range) => (
                        ...
                        >
                          <div
                            className={`w-5 h-5 border flex items-center justify-center transition-all duration-300 rounded-sm ${selectedPriceIds.includes(range.id) ? "bg-black border-black" : "border-gray-300 group-hover:border-gray-500"}`}
                          >
                            {selectedPriceIds.includes(range.id) && (
                              <div className="w-2.5 h-2.5 bg-white" />
                            )}
                          </div>
                          <span
                            className={`text-sm tracking-wide transition-colors ${selectedPriceIds.includes(range.id) ? "text-black font-semibold" : "text-gray-500 group-hover:text-black"}`}
                          >
                            {range.label}
                          </span>
                          <input
                            type="checkbox"
                            className="hidden"
                            checked={selectedPriceIds.includes(range.id)}
            ...
            <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 py-4 bg-white/95 backdrop-blur-md border-b border-gray-100 sticky top-28 z-20">
              ...
              <div className="flex items-center gap-6 w-full md:w-auto justify-end">
                ...
                <div className="flex items-center gap-3 group">
                  <span className="text-gray-400 text-xs uppercase font-bold hidden sm:inline group-hover:text-gray-600 transition-colors">
                    Sort:
                  </span>
                  <div className="relative">
                    ...
                    <ChevronDown
                      size={12}
                      className="absolute right-0 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none group-hover:text-black transition-colors"
                    />
                  </div>
                </div>
            ...
            <AnimatePresence mode="wait">
              {isLoading ? (
                ...
                >
                  ...
                  <button
                    onClick={clearAllFilters}
                    className="bg-black text-white px-8 py-3 rounded-full text-xs font-bold uppercase tracking-widest hover:bg-gray-800 transition-colors shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 duration-300"
                  >
                    Clear All Filters
                  </button>
                </motion.div>
...
const ProductCard = ({ product, addToCart, addToWishlist, isInWishlist }) => {
  ...
  return (
    ...
    >
      <div className="relative aspect-[3/4] overflow-hidden bg-[#F5F5F0] mb-4 rounded-sm transition-all duration-500 group-hover:shadow-[0_10px_30px_-10px_rgba(0,0,0,0.1)]">
        <Link
        ...
        >
          <img
            src={product.image}
            alt={product.name}
            className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-in-out ${hasSecondImage && isHovered ? "opacity-0 scale-105" : "opacity-100 scale-100"} ${!hasSecondImage && isHovered ? "scale-105" : ""}`}
            loading="lazy"
          />
          {hasSecondImage && (
            <img
              ...
              className={`absolute inset-0 w-full h-full object-cover transition-all duration-700 ease-in-out ${isHovered ? "opacity-100 scale-105" : "opacity-0 scale-100"}`}
              loading="lazy"
            />
          )}
        </Link>
        ...
        {product.onSale && (
          <div className="absolute top-3 left-3 bg-[#D4AF37] text-black text-[9px] font-bold uppercase px-3 py-1 tracking-[0.2em] shadow-sm z-20">
            Sale
          </div>
        )}
        ...
        <div className="absolute top-3 right-3 flex flex-col gap-2 z-20">
          ...
          <button
            ...
            className={`p-2.5 rounded-full backdrop-blur-md transition-all duration-300 shadow-sm ${isInWishlist
              ? "bg-white text-red-600 opacity-100 translate-x-0"
              : "bg-white/90 text-zinc-400 hover:text-red-600 hover:scale-105 opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0"
              }`}
          ...
          <button
            ...
            className="p-2.5 rounded-full backdrop-blur-md bg-white/90 text-zinc-400 hover:text-black hover:bg-[#D4AF37] transition-all duration-300 shadow-sm opacity-0 group-hover:opacity-100 translate-x-4 group-hover:translate-x-0 delay-75"
        ...
        <div className="absolute bottom-0 left-0 right-0 translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-out z-10">
          <button
            ...
            className="w-full bg-white/95 backdrop-blur-md text-black py-4 text-[10px] font-bold uppercase tracking-[0.2em] hover:bg-black hover:text-[#D4AF37] transition-colors border-t border-gray-100"
          >
            View Details
          </button>
        </div>
      ...
      <div className="text-center px-1 space-y-1">
        <Link to={`/product/${product.id || product._id}`}>
          <h3 className="font-serif text-lg text-gray-900 group-hover:text-[#D4AF37] transition-colors line-clamp-1">
            {product.name}
          </h3>
        </Link>
        ...
        {product.colors && product.colors.length > 0 && (
          <div className="flex justify-center items-center gap-1.5 py-1 min-h-[16px]">
