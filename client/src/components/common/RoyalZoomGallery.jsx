...
import { Search, X, ZoomIn, ZoomOut, Scan, Grid, ChevronLeft, ChevronRight, Heart, Share2 } from 'lucide-react';
...
const RoyalZoomGallery = ({ images, productName, product, portalId = 'zoom-portal' }) => {
        ...
        <div className="flex flex-col-reverse md:flex-row gap-4 h-full relative">
            ...
            <div className="relative flex-1 bg-white/5 border border-white/10 rounded-lg overflow-hidden md:h-[600px] flex items-center justify-center group z-10">
                ...
                <div
                    ...
                    onClick={() => { setIsModalOpen(true); setModalZoom(1); }}
                    ref={imgRef}
                >
                    <img
                        src={selectedImage}
                ...
                <div className="absolute top-4 right-4 flex flex-col gap-3 z-20">
                    <button
                        onClick={(e) => { e.stopPropagation(); isWishlisted ? removeFromWishlist(product.id) : addToWishlist(product); }}
                    ...
                    <button
                        onClick={(e) => { e.stopPropagation(); setIsModalOpen(true); setModalZoom(1); }}
                    ...
                    >
                        <Scan size={20} className="group-hover/scan:rotate-90 transition-transform" />
                    </button>
                </div>
            </div>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[100] bg-black/95 flex flex-col animate-in fade-in duration-300">
                    ...
                    <div className="flex items-center justify-between px-6 py-4 bg-black border-b border-white/10 z-50">
                        <h3 className="text-white font-serif text-lg tracking-wide">{productName}</h3>
                        <button onClick={() => setIsModalOpen(false)} className="p-2 bg-white/10 rounded-full hover:bg-white/20 text-white transition-colors">
                            <X size={24} />
                        </button>
                    </div>
                    ...
                    >
                        <div
                            ...
                            onWheel={(e) => setModalZoom(prev => Math.min(Math.max(1, prev + (e.deltaY * -0.001)), 4))}
                        >
                            <img
                                src={selectedImage}
                                alt="Full View"
                    ...
                    <div className="h-20 bg-black border-t border-white/10 flex items-center justify-center gap-6 z-50">
                        ...
                        <span className="text-royal-gold font-mono w-16 text-center">{Math.round(modalZoom * 100)}%</span>
